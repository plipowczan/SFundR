...
public class ItemCompletedEmailNotificationHandler : INotificationHandler<TimeItemApprovedEvent>
{
  ...
  public Task Handle(TimeItemApprovedEvent domainEvent, CancellationToken cancellationToken)
  {
    Guard.Against.Null(domainEvent, nameof(domainEvent));

    return _emailSender.SendEmailAsync("test@test.com", "test@test.com",
      $"{domainEvent.ApprovedItem.Comment} was completed.", domainEvent.ApprovedItem.ToString());
  }
}
